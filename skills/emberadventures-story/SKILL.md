...
`rewards` must always be an array of reward objects. Do not use old reward
bucket objects such as `rewards.unlock_locations`,
...
- Objective `completion_control`: `"player"`, `"ai"`, or `"choice"`. `"player"` means the verifier checks one selected player-action objective after a player-authored turn and can record progress without completing. `"ai"` means EmberAdventures checks after the narrator reply and completes only if the reply actually showed the required outcome. `"choice"` means EmberAdventures presents this objective as a forced choice menu, completes the owner objective immediately when the player selects one option from `choices[]`, and applies that option's rewards without AI verification.
