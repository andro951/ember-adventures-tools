...
choice. Use `state.reward_bundles` when the same exact reward or group of
...
avoid repeated reward JSON; use `state.reward_bundles` for deduplication.
...
- Do not use reward-only objectives to deduplicate repeated rewards; use `state.reward_bundles` for that.
...
- Repeated deterministic effects are factored into `state.reward_bundles`.
...
- `state.reward_bundles` for shared deterministic rewards that would otherwise
